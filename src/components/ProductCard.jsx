...
export default function ProductCard({ product, language, t, onViewDetails }) {
    return (
        <motion.article
        ...
        >
            <div className="grid aspect-square place-items-center bg-skysoft p-3 sm:aspect-[4/3] sm:p-5 dark:bg-[#3b3f45]">
                <div className="grid h-full w-full place-items-center rounded-2xl border border-dashed border-emerald-200 bg-white/70 px-2 text-center dark:border-mint/30 dark:bg-white/10">
                    <ImageIcon aria-hidden="true" className="mx-auto mb-2 text-emerald-500 sm:mb-3" size={28} />
                    <span className="text-xs font-bold text-slate-500 sm:text-sm dark:text-slate-300">{product.name[language]}</span>
                </div>
            </div>
            <div className="p-3 sm:p-6">
                ...
                <h3 className="mt-3 text-sm font-black leading-snug text-slate-950 sm:mt-4 sm:text-xl dark:text-white">{product.name[language]}</h3>
